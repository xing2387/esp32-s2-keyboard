    ...
    if (is_recv) {
        ...
        if (buttons & 0b00000100) {
            ...
            if (is_midkey && !is_pan) {
                // printf("send mid key\n");
                if (is_usb_connected) {
                    tud_hid_mouse_report(REPORT_ID_MOUSE, 0b00000100, 0, 0, 0, 0);
        ...
        if (is_usb_connected) {
            tud_hid_mouse_report(REPORT_ID_MOUSE, buttons & 0b00000011, dx, dy, pan_y, pan_x);
